...
char* Varint::Encode32(char* sptr, uint32 v) { return Encode32Inline(sptr, v); }
...
const char* Varint::Parse32BackwardSlow(const char* ptr, const char* base,
  ...
  if (prev == NULL) return NULL;  // no value before 'ptr'
...
const char* Varint::DecodeTwo32ValuesSlow(const char* ptr, uint32* a,
                                          uint32* b) {
  uint64 v = 0;
